...
**Resolved in CHORE-005 (PR #TBD):** the deferred wrap is now in place. `src/db/repos/cards.ts` exports `appendToPage(pageId, card)` and `detachFromPage(pageId, cardId)`, each wrapping the page lock check + `db.cards.add` / `db.cards.delete` + `db.pages.update({cardIds: …})` in a single `db.transaction('rw', [db.pages, db.cards], …)`. ListDetail's `onAddSuccess` / `onDelete` now call the atomic helpers; the prior two-step sequence is gone from the route. Failure-mode tests (repo + route layer) inject a `db.pages.update` rejection and assert rollback. `pages.update` itself remains unconditional, exactly as ADR-016 specifies — the lock invariant is still enforced at the route layer and inside the new atomic helpers, not on `pages.update`.
