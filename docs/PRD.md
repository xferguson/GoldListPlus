...
- After file selection, the app parses and validates the file **before** mutating anything. On any validation failure the user sees an inline error under the Import button with `role="alert"`; the database is untouched. Failure cases and copy:
  ...
  - Any row inside `books` / `pages` / `cards` / `reviews` fails per-field schema validation (missing required key, wrong type, enum violation on `tier` or `rating`, non-finite number, non-array where an array is required, or an `id` containing `#` / `?` / `/`) → the importer emits the `malformed-row` error variant carrying `{ table, index, reason }`. The user sees table- and row-specific copy with the table noun rephrased to match the rest of the UI (book / list / card / review) and the row index reported as 1-based:
    - `books` → **"This backup has a malformed book at row <index>. Nothing was imported."**
    - `pages` → **"This backup has a malformed list at row <index>. Nothing was imported."**
    - `cards` → **"This backup has a malformed card at row <index>. Nothing was imported."**
    - `reviews` → **"This backup has a malformed review at row <index>. Nothing was imported."**
  - The `reason` field on the `malformed-row` variant is developer-facing only (tests, future telemetry). It MUST NOT be interpolated into the user-facing string above.
